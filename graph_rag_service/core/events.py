...
from graph_rag_service.services.entity_extractor.hybrid_entity_extractor import HybridEntityExtractor
...
async def startup_event(config: Config) -> dict:
    ...
    logger.info("=" * 80)
    logger.info("🚀 Starting GraphDB RAG System with Entity Extraction")
    logger.info("=" * 80)
    ...
    try:
        ...
        connector = GraphDBConnector(
            ...
            database=config.graph_db.database
        )
        connector.connect()
        components["connector"] = connector
        logger.info(f"✓ Connected to GraphDB at {config.graph_db.uri}")
        ...
        try:
            ...
            """
            emb_result = connector.execute_query(embeddings_query)
            if emb_result:
...
async def shutdown_event(components: dict) -> None:
    ...
    1. Закрывает соединение с GraphDB
    2. Очищает кэши
    ...
    """
    logger.info("\n" + "=" * 80)
    logger.info("🛑 Shutting down GraphDB RAG System")
    logger.info("=" * 80)
...
async def cleanup_components(components: dict) -> None:
    ...
    if connector:
        try:
            ...
            logger.error(f"Error closing GraphDB connection: {e}")
...
async def health_check_event(components: dict) -> dict:
    ...
    try:
        ...
        if connector:
            ...
            health_status["components"]["graph_db"] = "healthy"
        else:
            health_status["components"]["graph_db"] = "not_initialized"
            health_status["overall"] = "degraded"
    except Exception as e:
        health_status["components"]["graph_db"] = f"unhealthy: {str(e)}"
...
async def warm_up_event(components: dict) -> None:
    ...
    try:
        ...
        connector = components.get("connector")
        if connector:
            logger.info("  • Warming up GraphDB connection...")
            # Несколько тестовых запросов
            connector.execute_query("RETURN 1")
            connector.execute_query("MATCH (n) RETURN count(n) LIMIT 1")
            logger.info("  ✓ GraphDB connection warmed up")
...
async def initialize_sample_data(components: dict) -> None:
    ...
    if not all([connector, graph_builder, vector_store]):
    ...
    try:
        ...
        sample_docs = [
            ...
            {
                "id": "sample_graphdb",
                "title": "Графовые базы данных и GraphDB",
                "content": """
                Графовые базы данных, такие как GraphDB, оптимизированы для хранения и обработки 
