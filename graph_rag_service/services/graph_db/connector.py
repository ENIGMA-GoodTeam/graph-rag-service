...
from graph_rag_service.core.exceptions import ServiceUnavailable, AuthError
...
class GraphDBConnector:
    """GraphDB Connector for managing connections and executing queries."""
    
    def __init__(self, uri: str, username: str, password: str, database: str = "graph_db"):
        self.uri = uri
    ...
    def connect(self) -> None:
        """Установка соединения с GraphDB"""
        try:
            ...
            logger.info(f"✓ Connected to GraphDB at {self.uri}")
        except AuthError as e:
            logger.error(f"❌ Authentication failed: {e}")
            raise
        except ServiceUnavailable as e:
            logger.error(f"❌ GraphDB service unavailable: {e}")
            raise
