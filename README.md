# 🚀 GraphDB GraphRAG System

Production-ready система для работы с RAG (Retrieval-Augmented Generation) на базе графовой базы данных GraphDB с семантическим кэшированием Redis.

## 📋 Возможности

### Основной функционал
- 🧠 **GraphRAG** - Retrieval-Augmented Generation с использованием графовой структуры GraphDB
...
- **Graph Database**: GraphDB 5.x с APOC плагинами
- **Vector Search**: GraphDB Vector Index (cosine similarity)
...
│   │   ├── graph_db/                  # GraphDB services
│   │   │   ├── connector.py  # Connection & queries
...
# GraphDB
GRAPH_DB_URI=bolt://graph_db:7687
GRAPH_DB_USERNAME=graph_db
GRAPH_DB_PASSWORD=password123
GRAPH_DB_DATABASE=graph_db
...
Это запустит:
- GraphDB (http://localhost:7474, bolt://localhost:7687)
...
**2. Запустите GraphDB и Redis в Docker**
```
# GraphDB
docker run -d \
    --name graph_db-local \
    -p 7474:7474 -p 7687:7687 \
    -e GRAPH_DB_AUTH=graph_db/password123 \
    -e GRAPH_DB_PLUGINS='["apoc"]' \
    graph_db:latest
...
```
# Остановить GraphDB
docker stop graph_db-local

# Остановить Redis
docker stop redis-cache

# Запустить снова
docker start graph_db-local
...
**Очистить граф GraphDB:**
```
# Через GraphDB Browser (http://localhost:7474)
...
# Только GraphDB
docker-compose logs -f graph_db
```
...
# С coverage
uv run pytest tests/ --cov=graph_rag_service --cov-report=html
```
...
### GraphDB не подключается
```
# Проверить статус
docker ps | grep graph_db

# Проверить логи
docker logs graph_db-local

# Перезапустить
docker restart graph_db-local
```
